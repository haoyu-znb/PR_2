...
def get_config(name):
    ...
    elif name == 'Scene' or name == 'Scene_15': # 兼容两个名字
        config = {
            'batch_size': 100,
            'lr': 0.001,
            'layer_num': 3,
    ...
    elif name == 'LandUse' or name == 'LandUse_21': # 兼容两个名字
        config = {
            'batch_size': 100,
            'lr': 0.001,
            'layer_num': 3,
    ...
    # --- [修改点] 把 'HW' 改成 'HandWritten' 以匹配你的指令 ---
    elif name == 'HandWritten' or name == 'HW': 
        config = {
            'batch_size': 100,
            'lr': 0.001,
            'layer_num': 3,
    ...
    # 安全检查：如果名字都没匹配上，抛出错误
    if config is None:
        raise ValueError(f"Dataset '{name}' not found in options.py! Please check the name.")
